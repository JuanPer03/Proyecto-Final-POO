...
public class NevpV {
/**
 * Metodo borrar de la clase NevpV del paquete login
 * Se encarga de sobrescribir la informacion del archivo que contiene a los vendedores
 * @param dump es de tipo Arrayist y contiene la informacion correspondiente al archivo el cual se desea sobreecribir
 */    
    void borrar(ArrayList <Vendedor> dump){
    File archivo = new File("Vendedor.dat");      
        if (archivo.delete()){
            System.out.println("Borrado");
        }        
        File newArchivo = new File("Vendedor.dat");
        for (int i = 0; i < dump.size(); i++) {
            try{
            Vendedor persona = new Vendedor();
            persona = dump.get(i);
